...
def load_samples(samples_path: Path, limit: int | None = None) -> list[dict]:
...
def main() -> None:
    ...
    sample_rows = load_samples(args.samples_path, limit=args.limit)
    model = build_model(
        gpu_mem_gib=15,
        cpu_mem_gib=96,
    ...
    for idx, row in enumerate(sample_rows):
        ...
        full_xyz, full_rot, full_seconds = run_mode(model, full_inputs, "full", seed=3000 + idx)
        fast_xyz, fast_rot, fast_seconds = run_mode(model, fast_inputs, "prefill_future_start", seed=3000 + idx)

        full_xy = extract_xy(full_xyz)
        fast_xy = extract_xy(fast_xyz)
        gt = gt_xy(data)

        full_axis = compute_axis_metrics(gt, full_xy)
        fast_axis = compute_axis_metrics(gt, fast_xy)
        ...
        result = {
            ...
            "full_ade_m": round(compute_ade(gt, full_xy), 4),
            "fast_ade_m": round(compute_ade(gt, fast_xy), 4),
            "ade_delta_m": round(compute_ade(gt, fast_xy) - compute_ade(gt, full_xy), 4),
            "full_longitudinal_mae_m": round(full_axis["longitudinal_mae_m"], 4),
            "full_lateral_mae_m": round(full_axis["lateral_mae_m"], 4),
            "fast_longitudinal_mae_m": round(fast_axis["longitudinal_mae_m"], 4),
            "fast_lateral_mae_m": round(fast_axis["lateral_mae_m"], 4),
    ...
    summary = {
        ...
        "mean_full_longitudinal_mae_m": round(float(np.mean([r["full_longitudinal_mae_m"] for r in results])), 4),
        "mean_full_lateral_mae_m": round(float(np.mean([r["full_lateral_mae_m"] for r in results])), 4),
        "mean_fast_longitudinal_mae_m": round(float(np.mean([r["fast_longitudinal_mae_m"] for r in results])), 4),
        "mean_fast_lateral_mae_m": round(float(np.mean([r["fast_lateral_mae_m"] for r in results])), 4),
        "mean_fast_vs_full_ade_m": round(float(np.mean([r["fast_vs_full_ade_m"] for r in results])), 4),
        "mean_fast_vs_full_fde_m": round(float(np.mean([r["fast_vs_full_fde_m"] for r in results])), 4),

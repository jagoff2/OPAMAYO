...
class Alpamayo1_5(ReasoningVLA):
    ...
    def sample_trajectories_from_data_with_vlm_rollout(
        ...
        **kwargs: Any,
        ...
        overall_start = time.perf_counter() if runtime_profile is not None else None
        skip_vlm_generation = kwargs.get("skip_vlm_generation", False)
        ...
        if skip_vlm_generation:
            prefill_start = time.perf_counter() if runtime_profile is not None else None
            ...
            if runtime_profile is not None and prefill_start is not None:
                runtime_profile["vlm_generate_seconds"] = time.perf_counter() - prefill_start
            ...
            generate_start = time.perf_counter() if runtime_profile is not None else None
            ...
            if runtime_profile is not None and generate_start is not None:
                runtime_profile["vlm_generate_seconds"] = time.perf_counter() - generate_start
            rope_deltas = self.vlm.model.rope_deltas

            # manually replace padding after EOS token
            generated_sequences = replace_padding_after_eos(
        ...
        if runtime_profile is not None:
            runtime_profile["expert_step_calls"] = 0
            runtime_profile["expert_step_seconds"] = 0.0
        ...
        def step_fn(
            ...
            step_start = time.perf_counter() if runtime_profile is not None else None
            # x: (B*, *action_dim)
            ...
            if runtime_profile is not None and step_start is not None:
                runtime_profile["expert_step_calls"] += 1
                runtime_profile["expert_step_seconds"] += time.perf_counter() - step_start
        ...
        diffusion_start = time.perf_counter() if runtime_profile is not None else None
        ...
        if runtime_profile is not None and diffusion_start is not None:
            runtime_profile["diffusion_seconds"] = time.perf_counter() - diffusion_start
        ...
        if kwargs.get("return_extra", False):
            extract_start = time.perf_counter() if runtime_profile is not None else None
            extra = extract_text_tokens(self.tokenizer, generated_sequences)
            ...
            if runtime_profile is not None and extract_start is not None:
                runtime_profile["extract_seconds"] = time.perf_counter() - extract_start
                runtime_profile["generated_sequence_length"] = int(generated_sequences.shape[1])
                runtime_profile["prefill_seq_len"] = int(prefill_seq_len)
                runtime_profile["total_seconds"] = time.perf_counter() - overall_start
            return pred_xyz, pred_rot, extra
        if runtime_profile is not None and overall_start is not None:
            runtime_profile["generated_sequence_length"] = int(generated_sequences.shape[1])
            runtime_profile["prefill_seq_len"] = int(prefill_seq_len)
            runtime_profile["total_seconds"] = time.perf_counter() - overall_start
